...
#
#
# 예를 들어 참외밭이 위 그림과 같은 모양이라고 하자. 그림에서 오른쪽은 동쪽, 왼쪽은 서쪽, 아래쪽은 남쪽, 위쪽은 북쪽이다. 이 그림의 왼쪽위 꼭짓점에서 출발하여,
...
result = (w * h) - (abs(arr[(wi - 1) % 6][1] - arr[(wi + 1) % 6][1]) * abs(arr[(hi - 1) % 6][1] - arr[(hi + 1) % 6][1]))
print(result * n)
