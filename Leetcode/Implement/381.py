...
class RandomizedCollection:
    ...
    def __init__(self):
        self.data = []
        self.data_map = collections.defaultdict(set)

    def insert(self, val: int) -> bool:
        self.data_map[val].add(len(self.data))
        self.data.append(val)
        return len(self.data_map[val]) == 1

    def remove(self, val: int) -> bool:
        if self.data_map[val]:
            last_val = self.data[-1]
            val_index = self.data_map[val].pop()
            self.data[val_index] = last_val
            self.data_map[last_val].add(val_index)
            self.data_map[last_val].discard(len(self.data) - 1)
            self.data.pop()
            return True
        return False
